...
export const useSystemAppearance = () => {
  ...
  useEffect(() => {
    ...
    const onChange = (event: any) => setAppearance(
...
export const getThemePreference = (ctx: any) => getCookie(ctx, 'theme', BASE_COOKIE_OPTIONS);

export const ThemeProvider = ({ children, ...props }: { [key: string]: any }) => {
  const [preferredTheme, setPreferredTheme] = useState(
    () => props.preferredTheme,
  );

  const setPreferredThemeWithSideEffects = useCallback((preference: any) => {
